...
| `grade` (D/C/B/A/S, `ItemGrade`) | any item | display/progression tier only — matches the pre-existing `rarity` (COMMON→D … LEGENDARY→S in `data/items.json`), no combat formula reads it directly. Defaults to `D` when absent from JSON. |
