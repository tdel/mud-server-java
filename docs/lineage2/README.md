...
Skill/saving-throw checks (`CharacterInstance.check`/`save`/`checkOrSave`), the `(score-10)/2` ability modifier those checks use, short/long rest rules, and XP/leveling progression are unchanged — only combat resolution (attack, skill damage, defense) and, since [combat.md](combat.md)'s "Health & Mana pools" section, max HP/mana and their regen moved to this model.
