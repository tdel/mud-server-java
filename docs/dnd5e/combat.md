...
The previous turn/initiative-based combat engine (`domain/combat/ActionEconomy`, `domain/combat/CombatEncounter`, `game/CombatEngine`, `game/CombatResult`) has been removed. Combat is being redesigned around a real-time model instead — no rounds, no action economy, no turn order. `attack`/`use` (`network/command/ingame/Attack.java`/`Use.java`) are currently no-op stubs pending the new design; this file will be rewritten once that design lands.
