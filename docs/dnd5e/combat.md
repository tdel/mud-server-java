...
`game/CombatEngine` currently drives monster aggro via zone entry (`GamePlayerEnteredCell`) rather than a full turn/initiative loop — if a proper round-and-initiative system is ever built, this file is the reference for the standard structure above.
