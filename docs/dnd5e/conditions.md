...
None of the conditions above are implemented — no paralysis/stun/etc., no saving throws. What *does* exist is a lighter-weight, unrelated mechanic: `domain/actor/component/ActiveEffects` (held by `AbstractCharacter`, so both `CharacterInstance` and `MonsterInstance` can carry one), a set of temporary numeric modifiers (`ModifiedStat`: `ATTACK_ROLL`, `ARMOR_CLASS`) applied by buff/debuff spells (Bless, Bane, Shield of Faith — see [spellcasting.md](spellcasting.md)). It has a full lifecycle: `game/BuffExpiryEngine` ticks every second and expires effects proactively (not just filtered lazily on read), `game/BuffTriggerListener` announces cast/expiry to the whole room, and expiring/applying effects on a player character persists to `character_active_effect` (`persistence/CharacterActiveEffectDao`) so they survive a reconnect. If real conditions are ever added, this table is still the canonical effect list to encode as a separate, richer system — `ActiveEffects` is not a substitute for it (no incapacitation, no interaction with the hex-grid movement system `domain/HexCoordinate`).
