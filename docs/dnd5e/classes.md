...
`primaryAbility`, weapon proficiency, and armor proficiency (including shields) are now implemented end-to-end: `data/class.json` carries all three per class, `ClassService` exposes them, and `GameWorld.createCharacter`/`CharacterDao` bake them onto `GamePlayer` at creation. `primaryAbility` is pure data (shown on the stats screen and during class selection, `network/message/ingame/GamePlayerStats`/`network/message/authed/ChooseClass`) — it has no effect on ability-score generation. Weapon and armor proficiency are actually enforced directly on `GamePlayer` (`tryAttack`/`check`/`save`), no separate combat/check service:
