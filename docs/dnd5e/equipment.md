...
`domain/item/ItemTemplate` + `data/items.json` back the item-template system, but weapon/armor combat stats (pAtk/mAtk/pDef/mDef/accuracyBonus/evasionBonus/critBonus) no longer follow the SRD damage-die/AC values above — see [../lineage2/equipment.md](../lineage2/equipment.md) for the current fields. The weapon category/properties tables above (Simple/Martial, Finesse, etc.) are pure SRD reference now — `WeaponCategory` and weapon proficiency were removed from the implementation. Currency conversions still matter directly for `PlayerInventory.trySpendGold`/gold-drop balancing — see also [magic-items.md](magic-items.md) for the SRD rarity/bonus system (superseded in-game by the same item fields).
