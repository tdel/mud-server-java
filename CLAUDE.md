...
- One `CommandHandler` class per in-game command verb under `network/command/{connected,charselect,ingame,lobby}`, auto-discovered by `CommandRegistry` (constructor-injected `List<CommandHandler>`, no manual registration), paired with a response `record` under `network/message/**`. Use `/add-command` to scaffold a new one.
