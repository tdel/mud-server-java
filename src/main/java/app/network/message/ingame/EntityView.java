...
public record EntityView(UUID id, String name, double x, double y, double heading, double speed, int currentHealth,
    ...
    public static EntityView of(AbstractCharacter character) {
        ...
        return new EntityView(character.getId(), character.getName(), character.getPosition().x(),
                character.getPosition().y(), character.getHeading(),
                MovementEngine.unitsPerSecond(character.getSpeed()), character.getCurrentHealth(),
