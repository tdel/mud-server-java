 ...
 * plus la liste des occupants (personnages/monstres/PNJ) ni des portails :
 * ceux-ci arrivent séparément, scopés à AWARENESS_RANGE, via
 * {@link EntityAppeared}/{@link PortalAppeared} poussés par
...
public record MapEnter(AbstractCharacter character) implements OutputJsonMessage {
    ...
    public void toJson(TcpJsonOutput output) {
        ...
        output.write("MapEnter", new Payload(map.getName(), map.getDescription(), self.x(), self.y(),
                character.getMotionSystem().getHeading(), waypoints));
    }
