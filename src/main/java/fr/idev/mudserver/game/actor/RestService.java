 ...
 * simultanément tous les joueurs actuellement en ligne
 * ({@link GameWorld#onlineCharacters()}), donc aucun objet de domaine unique ne
 * peut porter cette logique. {@code game.CombatEngine} est le précédent dans ce
 * projet pour un {@code @Service} qui mute directement plusieurs
 * {@code GameCharacter} avant de publier un événement.
 *
 * <p>
...
public class RestService {
     ...
     * dernier repos long. Sinon, chaque joueur en ligne (initiateur compris)
     * regagne {@code hitDie/2 + 1 + modificateur CON} PV — même formule que le gain
     * de PV au level-up ({@code CharacterService.onCharacterGainedXp}), et voit son
    ...
    public RestOutcome shortRest(GamePlayer initiator) {
        ...
        for (GamePlayer character : gameWorld.onlineCharacters()) {
     ...
     * chaque joueur en ligne regagne tous ses PV manquants, compteur de repos
     * courts remis à zéro.
     */
