...
public class ItemDao {
    ...
    public void insert(Item item) {
        jdbcTemplate.update(
                ...
                new MapSqlParameterSource()
                        .addValue("id", item.id())
                        .addValue("templateId", item.templateId())
                        .addValue("roomId", item.roomId())
                        .addValue("characterId", item.characterId())
    ...
    /**
     * Verrou pessimiste — équivalent du {@code LockMode::PESSIMISTIC_WRITE} PHP.
     * À n'appeler que dans une transaction (méthode appelante {@code @Transactional}) :
     * bloque toute autre transaction voulant lire/modifier cette ligne jusqu'au commit/rollback.
     */
    ...
    public List<Item> findByRoomId(UUID roomId) {
        return jdbcTemplate.query(
                "SELECT * FROM item WHERE room_id = :roomId",
    ...
    public List<Item> findByCharacterId(UUID characterId) {
        return jdbcTemplate.query(
                "SELECT * FROM item WHERE character_id = :characterId",
    ...
    public void assignToRoom(UUID itemId, UUID roomId) {
        jdbcTemplate.update(
                "UPDATE item SET room_id = :roomId, character_id = NULL, slot = NULL WHERE id = :itemId",
    ...
    public void updateSlot(UUID itemId, EquipmentSlot slot) {
        jdbcTemplate.update(
                "UPDATE item SET slot = :slot WHERE id = :itemId",
                new MapSqlParameterSource()
                        .addValue("itemId", itemId)
                        .addValue("slot", slot == null ? null : slot.name())
        );
    }
}
