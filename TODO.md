...
- **Tests** : couverture large sur `game`/`persistence`/`domain` (dont `CombatServiceTest`, `CombatEngineTest`, `CombatEncounterTest`, `GameMonsterTest`, `GamePlayerDeathTest`, `MonsterServiceTest`, `CharacterServiceTest`, `LevelServiceTest`), plus un premier test au niveau `network/command/**` (`CommandDispatcherTest`, verrouillage des verbes autorisés pendant un combat).
