...
- **Rareté et objets magiques** : enum `Rarity` DnD5e (COMMON→ARTIFACT) sur `ItemTemplate`, colorée en ANSI dans tous les messages telnet affichant un item (prise, dépôt, équipement, inventaire, description de room, butin, boutique PNJ). Bonus magique `+1`/`+2`/`+3` (champ `bonus`) sur l'arme/l'armure/le bouclier équipé, pas encore reconnecté à un jet d'attaque/CA depuis la suppression de l'ancien moteur de combat (voir item 1 ci-dessous) — pas de règle d'attunement (limite du nombre d'objets magiques utilisables simultanément), absente comme le reste des états/conditions.
- **Consommables** : commande `use <item>`, `ConsumableItem`/`ConsumableEffect` (comportement porté par l'item lui-même, pas par un Service externe). 4 paliers de potion de soin DnD5e (soin, supérieure, majeure, suprême — dés et prix RAW). Seul effet implémenté à ce jour : `HEALING` — le point d'extension existe déjà pour un futur poison/buff. `use` est actuellement un stub sans effet (le comportement en combat dépendait de l'ancien moteur, supprimé — voir item 1).
- **Combat** : l'ancien moteur tour par tour (`game/CombatEngine.java`, `domain/combat/CombatEncounter.java`, `domain/combat/ActionEconomy.java`, `game/CombatResult.java`) a été entièrement retiré en vue d'une refonte en temps réel (plus de tours, plus d'action economy). `attack` (`network/command/ingame/Attack.java`) ne fait plus que sélectionner une cible, sans résoudre d'attaque. Classe d'Armure réelle toujours calculée (`10 + mod DEX`, cappée selon la catégorie d'armure équipée, + bonus de bouclier + bonus magique) mais plus consommée par aucun jet d'attaque pour l'instant. Mort/respawn : le joueur réapparaît à PV pleins dans la salle de départ ; un monstre tué est retiré définitivement de sa salle, crédite l'XP au tueur et déclenche son butin (or garanti + objets à tirage indépendant par entrée, `game/actor/LootService.java`, transactionnel) — ce chemin ne dépend pas du moteur de combat et reste fonctionnel.
- **Monstres** : `MonsterInstance` (sous-type scellé de `AbstractCharacter`, distinct du joueur), gabarits + spawns fixes chargés au démarrage (`game/actor/MonsterService.java` + `data/monsters.json`, 7 gabarits). L'agressivité de zone (`getPresenceRadius` déclenchant un combat à l'entrée d'un joueur) dépendait de l'ancien moteur de combat, retiré avec lui — à reconcevoir avec le nouveau système temps réel. Ce qui manque encore : aucun déplacement/patrouille, pas de poursuite d'un joueur qui fuit, pas de décision tactique au-delà de la riposte.
...
- **Tests** : `src/test` ne contient aujourd'hui que `MonsterTemplateLootTest` et `CharacterInstanceLevelUpTest` — le reste de la couverture mentionnée plus bas dans ce document est obsolète et doit être reconstruit.

## Systèmes absents ou à l'état de simple champ de données

### 1. Combat — à reconstruire en temps réel
L'ancien socle tour par tour (jets d'attaque, initiative, action economy, agressivité de zone) a été retiré en vue d'une refonte en temps réel (plus de tours). À concevoir puis construire de zéro :
- **Résolution d'attaque en temps réel** : plus de jet d'initiative/tour — un modèle de cadence/cooldown par attaque reste à définir.
- **PvP** : absent — `Attack.java` ne résout plus aucune attaque (stub).
- **Respawn de monstre** : aucun — les spawns sont fixes et chargés une seule fois au démarrage.
- **IA de déplacement** : les monstres ne quittent jamais leur salle — pas de patrouille, pas de poursuite d'un joueur qui fuit, aucune décision tactique.
- **Agressivité de zone** : à reconcevoir avec le nouveau moteur (l'ancien hook `GamePlayerEnteredCell`/`presenceRadius` a été retiré avec `CombatEngine`).
...
### 4. États (conditions)
Absents (empoisonné, étourdi, paralysé, charmé, effrayé...). Dépend du nouveau moteur de combat temps réel (§1), pas encore construit.

### 5. Repos (courte/longue pause)
Absent — une première version (`rest short|long`) a été implémentée puis entièrement retirée avec l'ancien moteur de combat (le repos long y consommait des provisions et vérifiait `isInCombat()`). Les PV se restaurent aujourd'hui uniquement via la mort/respawn ou une potion de soin (`use`, actuellement un stub). À reconcevoir, probablement après le nouveau moteur de combat temps réel (§1) et les futurs emplacements de sorts (§3) qu'un repos aura vocation à restaurer. Le champ `CharacterInstance.shortRestCount` (colonne DB `short_rest_count`) et les items `FOOD`/`nutritionValue` restent en place, prêts à être réutilisés.
...
`CommandDispatcherTest` couvre le dispatcher (verrouillage des verbes pendant un combat) ; `Login`, `CharacterCreate`, `Take`, `Equip`, `Roll`, `Attack`, `Check`, `Save`, `Use` ont désormais chacun leur test dédié (voir item 5 de la dette technique ci-dessous). Les autres handlers (`Quit`, `Register`, `CharacterDelete`, `CharacterList`, `CharacterSelect`, `Drop`, `Examine`, `Go`, `Inventory`, `Look`, `Say`, `Select`, `Stats`, `Talk`, `Unequip`) restent sans test dédié — à garder en tête avant d'empiler de nouvelles commandes par-dessus une couche encore partiellement testée individuellement.
...
- Aucune protection brute-force sur le login (`network/command/connected/Login.java:70-77`) : ni compteur de tentatives, ni délai, ni verrouillage.
...
- Le TOCTOU de double-login déjà connu (`Login.java:76-79`) a un rayon d'impact plus large que prévu : `GamePlayer.equipItem` documente explicitement (javadoc, `GamePlayer.java:376-379`) que l'invariant « une connexion pilote un seul personnage » n'est pas garanti. S'il est un jour déclenché, deviennent réellement concurrents et non protégés : `PlayerInventory.addGold`/`trySpendGold` (`+=` non atomique, `PlayerInventory.java:29-42`), la liste d'inventaire, et `CharacterInstance.takeDamage` (aucun verrou depuis le retrait de l'ancien moteur de combat — à revoir avec le nouveau système temps réel). À l'inverse, `Room.tryClaimCell`/`join`/`leave` et `MonsterInstance.takeDamage` sont déjà correctement verrouillés.
...
- Cf. aussi la lacune transverse déjà notée plus haut sur la couverture par `CommandHandler` — les handlers les plus utilisés sont désormais couverts (item 5 ci-dessous), le reste ne l'est pas encore.
...
- `GamePlayer.java` (468 lignes, 40 membres publics) approche la taille de god-class vu sa centralité.
...
5. ~~Tests : couche telnet, `LootListener`, puis les `CommandHandler` les plus utilisés.~~ **[Résolu]** : `IacFilterDecoderTest`, `TelnetConnectionTest`, `TelnetSessionHandlerTest`, `TelnetServerInitializerTest` (couche telnet, via `EmbeddedChannel` et des doublons de test par sous-classement — pas de Mockito dans ce projet — `TelnetServer` volontairement laissé hors périmètre car `start()` bloque sur un vrai bootstrap réseau Netty, non testable unitairement ; nécessiterait un test d'intégration dédié à part) ; `LootServiceTest` (or garanti, drop 100 %/0 % indépendant par entrée, table vide, or + butin dans la même transaction) ; `LoginTest`, `CharacterCreateTest`, `TakeTest`, `EquipTest`, `RollTest`, `AttackTest`, `CheckTest`, `SaveTest` (couverture des `CommandHandler` les plus utilisés, via un `RecordingConnection` désormais mutualisé sous `network/command/RecordingConnection.java`, avec callback de prompt pour piloter les scénarios multi-étapes de `Login`/`CharacterCreate`).
