---
name: add-command
description: Scaffold a new in-game telnet command (ActionHandler + response message + registration) for the mud-server-java project. Use when adding a new player-facing verb like say, roll, go, look, equip.
---

Adding a new in-game command in this codebase means creating a matching pair of classes plus a registration entry. Follow the existing pattern under `network/action/{connected,authed,ingame}` and `network/message/**` — pick the connection-state subpackage (`connected`, `authed`, `ingame`) that matches when the command should be usable.
...
2. **ActionHandler**: create a class under `network/action/<state>/` implementing `ActionHandler`:
   - `name()` — the verb the player types.
   - `states()` — which `ConnectionState`(s) the command is valid in.
   - `onReceive(Session, String)` — parse the raw input, do the work (call into `game/` services, not persistence directly), and return the appropriate response record from step 1.

3. **Register it**: add the new handler to `ActionRegistry` alongside the other handlers for that state.

4. **Business logic**: if the command needs new game logic beyond simple lookups, put it in the relevant `game/` service (e.g. `GameWorld`, `ItemService`), not in the `ActionHandler` itself — handlers stay thin and dispatch-focused.
