---
name: add-command
description: Scaffold a new in-game telnet command (CommandHandler + response message) for the mud-server-java project. Use when adding a new player-facing verb like say, roll, go, look, equip.
---

Adding a new in-game command in this codebase means creating a matching pair of classes. Follow the existing pattern under `network/command/{connected,authed,ingame}` and `network/message/**` — pick the connection-state subpackage (`connected`, `authed`, `ingame`) that matches when the command should be usable.
...
2. **CommandHandler**: create a class under `network/command/<state>/` implementing `CommandHandler`:
   - `name()` — the verb the player types.
   - `states()` — which `ConnectionState`(s) the command is valid in.
   - `onReceive(Connection connection, String argument)` — `void`, not a return value. Parse the raw input, do the work, and call `connection.send(new SomeMessage(...))` directly (one or more times — see `CharacterDelete.onReceive` for a handler that sends an intermediate message plus a follow-up). Look at `network/command/ingame/Take.java` for a full example that also touches domain state.

3. **Registration**: none needed — annotate the class `@Component` and `CommandRegistry` auto-discovers it via Spring's constructor-injected `List<CommandHandler>`. Startup fails fast (`IllegalStateException`) if two handlers in the same state declare the same `name()`.
