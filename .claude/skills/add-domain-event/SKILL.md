...
2. **Publish it from the domain object**: add or extend a method on the owning domain object (e.g. `GamePlayer`) that:
...
5. **`@Order`**: only add it if message ordering to the player matters relative to another listener on the *same* event (e.g. a death broadcast must arrive before a level-up message — see `RoomService`/`CharacterService`'s `@Order(1)`/`@Order(2)` on `CharacterDied`). Most listeners don't need it.
