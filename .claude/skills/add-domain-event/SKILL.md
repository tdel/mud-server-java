...
3. **Listen for it**: add an `@EventListener` method in the relevant persistence listener (`persistence/listener/CharacterPersistenceListener`, `ItemPersistenceListener`, `WorldInstancePersistenceListener` — pick by what the event is about, not by who published it). The listener's job is to reflect the already-applied fact:
